...
// Overall size of the head in the 3D view. 1.0 = default. Turn this up if
// the head looks too small, down if it looks too big. Just edit this
// number, save the file, and refresh the browser — no need to restart the
// Python server, this file is loaded fresh by the browser each time.
...
function initThreeHead(container) {
  ...
  function addPlaceholder() {
    ...
    headGroup.scale.setScalar(MODEL_SCALE);
    render();
  }
  ...
  function loadCustomModel() {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        const scale = (1.7 / maxDim) * MODEL_SCALE; // ~match placeholder's 1.7-unit height, adjustable via MODEL_SCALE
...
// Build the 5 track buttons (audio_1.wav .. audio_5.wav in static/audio/).
// Each is checked for existence so missing files show as disabled rather
// than silently failing when a session starts.
const TRACK_COUNT = 5;
function initTrackButtons() {
  const container = document.getElementById("audio-track-buttons");
  for (let i = 1; i <= TRACK_COUNT; i++) {
    const btn = document.createElement("button");
    btn.textContent = `Audio ${i}`;
    btn.className = "track-btn";
    btn.dataset.track = i;
    const path = `/audio/audio_${i}.wav`;
    ...
    fetch(path, { method: "HEAD" })
      ...
      .catch(() => {
        btn.disabled = true;
        btn.title = `audio_${i}.wav not found in static/audio/`;
      });

    btn.addEventListener("click", () => selectTrack(i, path, btn));
    container.appendChild(btn);
  }
}

function selectTrack(num, path, btnEl) {
  document.querySelectorAll(".track-btn").forEach((b) => b.classList.remove("selected"));
  btnEl.classList.add("selected");
  selectedTrackNum = num;
  ...
  audioPlayer.addEventListener(
    ...
    () => {
      ...
      document.getElementById("audio-duration-label").textContent =
        `audio_${num}.wav (${audioDuration.toFixed(0)}s)`;
